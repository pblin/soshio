...
def push(posts):
    ...
    def try_push(message, retry=True):
        try:
            ...
            if not retry:
                logging.error('Error <{error}> raised while pushing post <{id}>. Skipping...'.format(error=error.message, id=id))
                return
            logging.warning('Error <{error}> raised while pushing post <{id}>. Retrying...'.format(error=error.message, id=id))
